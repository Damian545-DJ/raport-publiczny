...
COPY = {
    "pl": {
        ...
        "footer": "© 2026. Publiczna wersja jest zanonimizowana. Materiał ma charakter informacyjny i służy niezależnej weryfikacji dokumentów. Nie stanowi porady prawnej, prawomocnego rozstrzygnięcia ani przesądzenia odpowiedzialności którejkolwiek ze stron. Pełniejsze materiały mogą zostać udostępnione wyłącznie właściwym instytucjom lub zweryfikowanym odbiorcom w uzasadnionym i bezpiecznym trybie.",
    },
    "en": {
        ...
        "footer": "© 2026. The public version is anonymized. The material is informational and intended for independent verification of documents. It does not constitute legal advice, a final legal determination, or a finding of liability against any party. Fuller materials may be shared only with competent institutions or verified recipients through a justified and secure channel.",
    },
    "nl": {
        ...
        "footer": "© 2026. De publieke versie is geanonimiseerd. Het materiaal heeft een informatief karakter en dient voor onafhankelijke verificatie van documenten. Het vormt geen juridisch advies, geen definitieve juridische beslissing en stelt geen aansprakelijkheid van enige partij vast. Uitgebreidere stukken kunnen alleen worden gedeeld met bevoegde instanties of geverifieerde ontvangers via een gemotiveerd en veilig kanaal.",
    },
}
...
WRAPPER = re.compile(r'<div\b[^>]*\bclass=["\'][^"\']*\b(?:wrap|container)\b[^"\']*["\'][^>]*>', re.I)
...
def matching_div_close(text: str, opening: re.Match[str]) -> int | None:
    depth = 1
    for match in re.finditer(r"</?div\b[^>]*>", text[opening.end():], re.I):
        tag = match.group(0)
        if tag.startswith("</"):
            depth -= 1
            if depth == 0:
...
def convert_topnav(text: str, label: str) -> str:
    ...
    return text[:match.start()] + f"<nav{attrs}>{match.group(2)}</nav>" + text[match.end():]


def label_navs(text: str, label: str) -> str:
    def repl(match: re.Match[str]) -> str:
        attrs = match.group(1)
        if re.search(r"\baria-label=|\baria-labelledby=", attrs, re.I):
            return match.group(0)
        return f'<nav aria-label="{label}"{attrs}>'
...
def repair_generic_aria(text: str) -> str:
    ...
    return re.sub(r"<(div|span)\b((?=[^>]*\baria-label=)[^>]*)>", repl, text, flags=re.I)


def ensure_main(text: str, path: Path) -> str:
    ...
    if re.search(r"<main\b", text, re.I):
        def repl(match: re.Match[str]) -> str:
            ...
            if not re.search(r"\btabindex=", attrs, re.I):
                attrs += ' tabindex="-1"'
            return f"<main{attrs}>"
        return re.sub(r"<main\b([^>]*)>", repl, text, count=1, flags=re.I)
    ...
    wrapper = WRAPPER.search(text, body.end())
    wrapper_close = matching_div_close(text, wrapper) if wrapper else None
...
def add_skip(text: str, code: str) -> str:
    if 'class="skip-link"' in text:
        return text
    return re.sub(r"(<body\b[^>]*>)", rf'\1\n  <a class="skip-link" href="#main-content">{COPY[code]["skip"]}</a>', text, count=1, flags=re.I)


def add_footer(text: str, code: str) -> str:
    if re.search(r"<footer\b", text, re.I):
        return text
    footer = f'  <footer class="legal-footer" aria-label="{COPY[code]["footer_label"]}">\n    <p>{COPY[code]["footer"]}</p>\n  </footer>\n'
    return re.sub(r"</body>", footer + "</body>", text, count=1, flags=re.I)


def add_css(text: str, path: Path) -> str:
...
def fix_doc(text: str, code: str) -> str:
    text = re.sub(r'(<(?:meta|link)\b[^>]*?)\s*/>', r"\1>", text, flags=re.I)
    text = re.sub(r'<div\s+class="top">(.*?)</div>', rf'<nav class="top" aria-label="{COPY[code]["doc_nav"]}">\1</nav>', text, count=1, flags=re.I | re.S)
    text = text.replace('<article class="markdown-body md" id="content">Ładuję…</article>', '<main class="markdown-body md" id="main-content" tabindex="-1">Ładuję…</main>')
    text = text.replace('getElementById("content")', 'getElementById("main-content")')
    return text


def remove_generated_footer_css() -> None:
    path = ROOT / "assets" / "public-polish.css"
    text = path.read_text(encoding="utf-8")
    marker = "/* Legal footer added automatically per language */"
...
def process(path: Path) -> None:
    ...
    if path == ROOT / "doc.html":
        text = fix_doc(text, code)
    text = convert_topnav(text, COPY[code]["nav"])
    text = label_navs(text, COPY[code]["nav"])
    text = repair_generic_aria(text)
    text = ensure_main(text, path)
    text = add_skip(text, code)
    text = add_footer(text, code)
    text = add_css(text, path)
    text = encode_ampersands(text)
    path.write_text(text, encoding="utf-8")
