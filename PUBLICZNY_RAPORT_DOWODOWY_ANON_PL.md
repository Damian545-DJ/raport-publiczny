...
**Zakres:** zarzuty dot. naruszeń pracowniczych + zarzuty dot. prywatności w zakwaterowaniu agencyjnym + problemy z obsługą prawną  
...
Ten dokument opisuje fakty i wnioski **na podstawie posiadanych dowodów** (dokumenty, korespondencja, nagrania audio i wideo).

Sformułowania typu „oszustwo”, „manipulacja”, „wprowadzanie w błąd” oznaczają **podejrzenie i ocenę autorów wynikającą z przedstawionych faktów**, a nie prawomocny wyrok w sprawie karnej.

Wersja jest anonimizowana: usunięto imiona/nazwiska osób prywatnych, numer pokoju, nazwę i adres zakwaterowania oraz inne dane identyfikujące. Publikacja ma charakter informacyjny i społeczny. Repozytorium nie stanowi porady prawnej.

---

## 1) Streszczenie sprawy (sedno)

Sprawa dotyczy **zarzutów** o poważne nieprawidłowości w obszarze praw pracowniczych i rozliczeń, w tym m.in.:

- pozostawienia Pracownicy B bez środków do życia przez prawie pół roku (według posiadanej dokumentacji),
- naliczania „długu” za zakwaterowanie, który co tydzień wzrastał i osiągnął ok. 1000 EUR (według dokumentów/rozliczeń),
- możliwego naruszenia warunków umowy w fazie C (czas nieokreślony) z gwarancją 32 godzin tygodniowo,
- systemowych nieprawidłowości płacowych i rozliczeniowych wobec Pracownika A (według analiz dokumentów),
...
Dodatkowo opisano poważne problemy z obsługą prawną: brak przekazania kluczowych dokumentów, długie okresy ciszy, sprzeczne komunikaty oraz faktury bez jasnego wykazania wykonanych czynności (wg korespondencji i dokumentów rozliczeniowych).
...
- nagrania audio ze spotkań z agencją (presja, próby wymuszeń – zgodnie z opisem i treścią nagrań),
...
**Uwaga techniczna:** w repozytorium (GitHub) materiały powinny być dodawane jako załączniki w sposób bezpieczny (zamazanie danych wrażliwych, brak publikacji twarzy osób postronnych, itd.). Pełne wersje dokumentów należy przekazywać instytucjom/pełnomocnikom na żądanie.
...
- Przed Nowym Rokiem 2025: po zapowiedzi zgłoszenia podejrzenia „oszustwa” Prawnik 2 oddzwania; autorzy proszą o uzyskanie rozstrzygnięcia bezpośrednio z sądu – według relacji do dziś brak efektu oraz brak zwrotu wpłaconych pieniędzy; zamiast tego pojawiają się niejasne faktury „do dopłaty” (autorzy opisują to jako podejrzenie nieprawidłowości rozliczeniowych).

---

## 4) Zarzuty dot. naruszeń pracowniczych – Pracownica B (faza C, 32h)

Z raportu źródłowego i dokumentacji ma wynikać, że:

- Pracownica B została pozostawiona bez środków do życia przez prawie pół roku.
- Następnie naliczano jej „dług” za zakwaterowanie, który rósł co tydzień i osiągnął ok. 1000 EUR.
- Agencja mogła naruszyć warunki umowy fazy C (czas nieokreślony) z gwarancją 32h tygodniowo.
- Pracownica B była wzywana do biura, gdzie – zamiast rozwiązania sprawy/umowy – miało dojść do próby kolejnego nadużycia (autorzy wskazują, że mają na to dowody).

---

## 5) Zarzuty dot. naruszeń pracowniczych – Pracownik A (obraz systemowy)
...
- działania opisywane jako „manipulowanie dokumentacją”, które – według autorów – miały ukryć uchybienia (sformułowanie jako podejrzenie na podstawie analizy dokumentów).
...
- po stronie agencji miała występować niedopuszczalna presja, a nawet zawoalowane groźby i taktyki zastraszania (ocena autorów na podstawie treści nagrań),
- autorzy oceniają to jako próbę obejścia gwarancji wynikających z fazy C oraz naruszenie swobody woli, godności i poczucia bezpieczeństwa pracownika.

---

## 7) Zakwaterowanie agencyjne – wejścia osób trzecich do zamkniętego pokoju (wideo)

Z kolejnych dowodów (nagrania wideo) ma wynikać, że osoby trzecie wielokrotnie:
...
**Wnioski dowodowe z raportu:** materiał może stanowić podstawę do żądania od podmiotu zarządzającego zakwaterowaniem: procedur wejść do pokoi, listy osób z dostępem do kluczy/master key, rejestrów/logów wejść oraz protokołów ewentualnych kontroli dla wskazanych dat i godzin.
...
- później informacja o „ugodzie i że nic się nie da zrobić”, bez pokazania autorom dokumentu,
...
Z raportu źródłowego ma wynikać, że autorzy posiadają wielowarstwowy materiał dowodowy, który **może wskazywać** na:

- poważne naruszenia praw pracowniczych (faza C / 32h, potrącenia, niewypłacanie, urlopy, terminowość i rozliczenia),
- presję i próby wymuszeń (audio),
- naruszenia prywatności i bezpieczeństwa w zakwaterowaniu agencyjnym (wideo),
- poważne problemy z obsługą prawną (brak dokumentów, brak kontaktu, sprzeczne komunikaty, faktury bez jasnych podstaw).
